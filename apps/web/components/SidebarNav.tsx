...
function ErrorWatchIcon({ className = "w-5 h-5" }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
...
function AlertFlowIcon({ className = "w-5 h-5" }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
...
function DepWatchIcon({ className = "w-5 h-5" }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              ...
              <Link
                ...
                className={`flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all duration-200 ${
                  pathname.startsWith('/dashboard/admin')
                    ? 'bg-gradient-to-r from-red-600/20 to-orange-600/10 text-white border-l-2 border-red-500'
                    : 'text-red-400/80 hover:text-red-400 hover:bg-red-900/20'
                }`}
              >
                <AdminIcon className={`w-5 h-5 ${pathname.startsWith('/dashboard/admin') ? 'text-red-400' : ''}`} />
                <span className={`text-sm ${pathname.startsWith('/dashboard/admin') ? 'font-medium' : ''}`}>Admin</span>
              </Link>
            </li>
          )}
        </ul>
